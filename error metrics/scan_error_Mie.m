...
for ii=1:size(dz_array,2)
ii

Box = [40*um, 40*um, 20*um];
dr  = [60*nm, 60*nm, dz_array(ii)]; % 80*nm
...
U_err_MLB(ii) = norm(U_MLB_xy-U_Mie, 2)/norm(U_Mie, 2);
U_err_MLR(ii) = norm(U_MLB_xy-U_Mie, 2)/norm(U_Mie, 2);
U_err_MSR(ii) = norm(U_MLB_xy-U_Mie, 2)/norm(U_Mie, 2);
U_err_MLB2(ii) = norm(U_MLB2_xy-U_Mie, 2)/norm(U_Mie, 2);
U_err_MLB4(ii) = norm(U_MLB4_xy-U_Mie, 2)/norm(U_Mie, 2);

end

figure
hold on
plot(dz_array./lambda, U_err_MLB,'-o')
plot(dz_array./lambda, U_err_MLR,'-+')
plot(dz_array./lambda, U_err_MSR,'-x')
plot(dz_array./lambda, U_err_MLB2,'-square')
plot(dz_array./lambda, U_err_MLB4,'-diamond')
xlabel('dz(\lambda)')
