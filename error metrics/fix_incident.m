...
uin = exp(1i*2*pi*fxin*X)*exp(-1i*2*pi*(z(round(end/2))-z(1)+dr(3))*cos(thetain)/lambda);
...
% Correct ouput wave
uin_xy = exp(1i*2*pi*fxin*X)*exp(-1i*2*pi*(z(round(end/2))-z(1)+dr(3))*cos(thetain)/lambda)*exp(1i*2*pi*(z(end)-z(1)+dr(3))*cos(thetain)/lambda);
...
U_Mie = ScalarMieField(X,Y,z,thetain,lmax,n_imm,nsphere,k0,rad);
