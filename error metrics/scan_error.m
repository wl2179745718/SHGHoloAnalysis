...
dx_array = [0.02:0.02:0.14]*um;


for ii=1:size(dx_array,2)
ii

Box = [40*um, 40*um, 20*um]; %40*um
dr  = [dx_array(ii), dx_array(ii), 80*nm];
...
figure_window = [-8*sigma/um 8*sigma/um];
...
U_err_MLB(ii) = norm(U_MLB_xy-U_MOM, 2)/norm(U_MOM, 2);
U_err_MLR(ii) = norm(U_MLB_xy-U_MOM, 2)/norm(U_MOM, 2);
U_err_MSR(ii) = norm(U_MLB_xy-U_MOM, 2)/norm(U_MOM, 2);
U_err_MLB2(ii) = norm(U_MLB2_xy-U_MOM, 2)/norm(U_MOM, 2);
U_err_MLB4(ii) = norm(U_MLB4_xy-U_MOM, 2)/norm(U_MOM, 2);

end

figure
hold on
plot(dx_array./lambda, U_err_MLB,'-o')
plot(dx_array./lambda, U_err_MLR,'-+')
plot(dx_array./lambda, U_err_MSR,'-x')
plot(dx_array./lambda, U_err_MLB2,'-square')
plot(dx_array./lambda, U_err_MLB4,'-diamond')
xlabel('dx(\lambda)')
legend('err_{MLB}','err_{MLR}','err_{MSR}','err_{MLB2}','err_{MLB4}')
set(gcf, 'Position', get(0, 'Screensize'));
ylim([0 1])