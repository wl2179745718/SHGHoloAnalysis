...
ps_array = 0.05:0.02:0.21;%[0.05, 0.1, 0.15, 0.2];
error_MLB_l2 = zeros(size(ps_array));
error_MLB_max= zeros(size(ps_array));
error_MLR_l2 = zeros(size(ps_array));
error_MLR_max= zeros(size(ps_array));
error_MSR_l2 = zeros(size(ps_array));
error_MSR_max= zeros(size(ps_array));
error_MSL_l2 = zeros(size(ps_array));
error_MSL_max= zeros(size(ps_array));

for ii = 1:size(ps_array,2)

ps          = ps_array(ii);       % pixel size (x,y,z) in object space (microns)
...
k0=(2*pi)/lambda;
k=k0*n_imm;
N           = [round(2^10*0.1/ps)-1, round(2^10*0.1/ps)-1, 2^9];                  % lateral pixel dimension 
L = ps*N;
delta = [ps, ps, ps];
deltaf = 1./L;
NA_in = 0.6;
...
fprintf('calculating fields based on multi-slice methods... \n')
...
fprintf('calculating fields based on Mie scattering... \n')
...
figure
subplot(2,4,1)
plot(ps_array,error_MLB_l2,'o');
title('MLB')
subplot(2,4,2)
plot(ps_array,error_MLR_l2,'o');
title('MLR')
subplot(2,4,3)
plot(ps_array,error_MSR_l2,'o');
title('MSR')
subplot(2,4,4)
plot(ps_array,error_MSL_l2,'o');
title('MSL')
subplot(2,4,5)
plot(ps_array,error_MLB_max,'o');
subplot(2,4,6)
plot(ps_array,error_MLR_max,'o');
subplot(2,4,7)
plot(ps_array,error_MSR_max,'o');
subplot(2,4,8)
plot(ps_array,error_MSL_max,'o');
