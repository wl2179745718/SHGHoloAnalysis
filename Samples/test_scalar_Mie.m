...
[theta1D, R] = XYz2thetaR(d, X, Y, z(end));
...
for ii = 1:length(theta1D)
    ...
    E_sca_scalar_Mie_1D(ii) = sum(blhl.*Pl);
end

E_sca_scalar_Mie = reshape(E_sca_scalar_Mie_1D, size(X));
...
figure
subplot(2,2,1)
imagesc(x,y,abs(E_sca_scalar_Mie))
axis square
colorbar
subplot(2,2,2)
...
colorbar
subplot(2,2,3)
imagesc(x,y,angle(E_sca_scalar_Mie))
axis square
colorbar
subplot(2,2,4)
