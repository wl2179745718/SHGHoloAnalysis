...
dGk = 1;
Eps = n_imm^2/lambda^2*0.0025;
Gdz2  =G_kx_ky(FX,FY,n_imm,lambda,  dr(3),dGk,Eps)./dr(1)./dr(2);
...
scaling = max(abs(Gdz(:,round(end/2))));
figure
hold on
plot(fx.*lambda, abs(Gdz(:,round(end/2)))./scaling, 'color', 'red','LineWidth',4);
plot(fx.*lambda, abs(Gdz1(:,round(end/2)))./scaling,'color', 'black','LineWidth',4);
plot(fx.*lambda, abs(Gdz2(:,round(end/2)))./scaling, '--','color', 'black','LineWidth',4);
