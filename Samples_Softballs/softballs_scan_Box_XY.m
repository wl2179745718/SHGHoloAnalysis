...
for jj = 1:size(box_array,2)
    box_array(jj)

Box = [box_array(jj), box_array(jj), 10];
dr  = [0.04, 0.04, 0.02];
dz1 = 0.01;
figure_window = 20;
...
titlestring = ['phase error L = ',num2str(box_array(jj))];
sgtitle(titlestring)
...
figure
hold on
plot(box_array, err_pwr_ratio_Oli)
plot(box_array, err_pwr_ratio_MLB)
xlabel('box size (transverse)')
ylabel('error')
legend('Olivier','MLB')
