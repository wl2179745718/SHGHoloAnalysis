...
dn_array = 0.01:0.03:0.28;

for jj = 1:size(dn_array,2)
    dn_array(jj)
...
titlestring = ['phase error dz = ',num2str(dn_array(jj))];
sgtitle(titlestring)
...
figure
hold on
plot(dn_array, err_pwr_ratio_Oli)
plot(dn_array, err_pwr_ratio_MLB)
xlabel('dn')
ylabel('error')
legend('Olivier','MLB')
