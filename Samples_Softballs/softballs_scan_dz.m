...
dz_array = 0.01:0.003:0.04;

for jj = 1:size(dz_array,2)
    dz_array(jj)

Box = [220, 220, 5];
dr  = [0.04, 0.04, dz_array(jj)];
dz1 = 0.01;
figure_window = 20;
...
% The incident field at the particle changes with the size of the box
Er = exp(1i*2*pi*(0-z(1)+dr(3))*cos(thetain)/lambda)*Er;
...
subplot(2,3,2)
imagesc(x,y,abs(E_MLB_image_Oli-Er))
axis square
colorbar
title('|E_{Oli} - E_{anal}|')
xlim(figure_window)
ylim(figure_window)
xlabel('x')
ylabel('y')
...
subplot(2,3,5)
imagesc(x,y,abs(E_MLB_image-Er))
axis square
colorbar
title('|E_{MLB} - E_{anal}|')
xlim(figure_window)
ylim(figure_window)
xlabel('x')
ylabel('y')
...
subplot(2,3,3)
imagesc(x,y,wrapToPi(angle(E_MLB_image_Oli- Er)))
axis square
colorbar
title('\angle ( E_{Oli} - E_{anal} )')
xlim(figure_window)
ylim(figure_window)
clim([-pi pi])
xlabel('x')
...
subplot(2,3,6)
imagesc(x,y,wrapToPi(angle(E_MLB_image- Er)))
axis square
colorbar
title('\angle ( E_{MLB} - E_{anal} )')
xlim(figure_window)
ylim(figure_window)
clim([-pi pi])
xlabel('x')
...
titlestring = ['phase error dz = ',num2str(dz_array(jj))];
sgtitle(titlestring)
...
figure
hold on
plot(dz_array, err_pwr_ratio_Oli)
plot(dz_array, err_pwr_ratio_MLB)
xlabel('dz')
ylabel('error')
legend('Olivier','MLB')
...
p_Oli = polyfit(dz_array(2:end), err_pwr_ratio_Oli(2:end), 2);
p_MLB = polyfit(dz_array, err_pwr_ratio_MLB, 2);

f_Oli = polyval(p_Oli, dz_array);
f_MLB = polyval(p_MLB, dz_array);

plot(dz_array, f_Oli, 'o')
plot(dz_array, f_MLB, 'o')
...
a_Oli = sum(dz_array.^2 .* err_pwr_ratio_Oli) / sum(dz_array.^4);
a_MLB = sum(dz_array.^2 .* err_pwr_ratio_MLB) / sum(dz_array.^4);

y_Oli = a_Oli * dz_array.^2;
y_MLB = a_MLB * dz_array.^2;

plot(dz_array, y_Oli, 'o')
plot(dz_array, y_MLB, 'o')