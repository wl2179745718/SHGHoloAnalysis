...
dz_array = 0.01:0.01:0.04;
box_array = 40:60:220;

err_pwr_ratio_Oli = zeros( size(box_array,2), size(dz_array,2) );
err_pwr_ratio_MLB = zeros( size(box_array,2), size(dz_array,2) );

for ii = 1:size(box_array,2)
for jj = 1:size(dz_array,2)
    box_array(ii)
    dz_array(jj)

Box = [box_array(ii), box_array(ii), 5];
...
ArrayF = 0;
for kk = 1:size(rj, 2)
ArrayF = ArrayF + exp(1i*(rj(1,kk)*(ki(1)-KX)+rj(2,kk)*(ki(2)-KY)+rj(3,kk)*(ki(3)-KZ)  ));
...
err_pwr_ratio_Oli(ii,jj) = error_power_Oli/Er_power;
err_pwr_ratio_MLB(ii,jj) = error_power_MLB/Er_power;

end
end

Oli_MLB_ratio = log10( err_pwr_ratio_Oli ./ err_pwr_ratio_MLB );

figure
hold on
imagesc( dz_array,box_array, Oli_MLB_ratio)
colorbar
xlabel('dz')
ylabel('L')
...
figure
hold on
imagesc( dz_array,box_array, err_pwr_ratio_Oli)
colorbar
xlabel('dz')
ylabel('L')

sgtitle('error: Oliviers method')
set(gcf, 'Position', get(0, 'Screensize'));

figure
hold on
imagesc( dz_array,box_array, err_pwr_ratio_MLB)
colorbar
xlabel('dz')
ylabel('L')
