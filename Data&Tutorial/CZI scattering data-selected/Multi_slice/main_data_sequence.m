...
data_origin      = 'D:\Shwetadwip\matlab_code\Data\Dataset_07\';
save_recon       = true;

init_unlocbox();

%% load path from which to load raw measurements
sample = load([data_origin '/sample_930_v1.mat'],'acq');
bkgd = load([data_origin '/bkgd_930.mat'],'acq');
disp('finished loading measurements');

%% load illumination angles
load([data_origin '/k_illumination_calibrated_930.mat']);
...
ps          = dpix_c./mag;       % pixel size (x,y,z) in object space (micron)
...
patchesToReconstruct = [2:4,6:15,17:20,23:25];

for patchFOV_index = patchesToReconstruct(randperm(length(patchesToReconstruct)))
    
    % patchFOV_index = 19;                    % user-input on which patch to reconstruct
...
%     sliderDisplayImVC2(amp_acqs); colormap gray; axis equal; caxis([0 3]);
    ...
    % hitList         = [];%[10:12,19:23,31:36,47:51,66:70,87:91,123:133,147:150];
    fx_in_crp       = fx_in;
    fy_in_crp       = fy_in;
    ...
    O               = 240;                  % axial dimension size of reconstruction space
    ...
    regParam        = 1.5e-3;                 % regularization parameter for 3D proxTV
...
%     figure('Name','Reconstruction result');
    ...
    for iter = 1:maxiter
        ...
        a = toc;
        fprintf('iteration: %d,  error: %5.5e,  elapsed time: %5.2f seconds,  patch# : %2.2d\n',iter, cost(iter), a, patchFOV_index);
    ...
    if save_recon
        fileN = sprintf('recon_910_v1_patch%0.2d.mat',patchFOV_index);
        savePath = [data_origin 'reconstructions_v1\mat\' fileN];
