function [] = Diagnose_Greens_funtion_integral_NA_2D_scan(k0, lambda, NA, Boxsize, dz1_array)

Rela_error = zeros(size(dz1_array, 2),size(Boxsize,2));
for ii=1:size(dz1_array, 2)
    parfor jj=1:size(Boxsize,2)

Box = [Boxsize(jj), Boxsize(jj), 5];
dr  = [0.04, 0.04, 0.01];
dz1 = dz1_array(ii);
figure_window = 20;
...
figure
imagesc(Boxsize,log10(dz1_array),Rela_error)
xlabel('Box size')
