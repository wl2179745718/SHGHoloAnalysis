...
%%
function [an,bn,RCSTheta,ETheta] = mieHKURCS(a,f,erb,urb,erp,urp,N,theta)
format long;
theta(theta == 0) = 1e-7;
...
ETheta(1,j)=EsThetaTemp;
end 
RCSTheta=(abs(ETheta).^2).*4.*pi./kb./kb;
RCSTheta = RCSTheta./max(RCSTheta);
